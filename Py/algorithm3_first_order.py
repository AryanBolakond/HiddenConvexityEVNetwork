"""
Algorithm 3: first-order re-optimization of station-level (SP-j) for a
FIXED topology (x, lambda_bar), for fast re-solves under updated demand
without re-running Algorithm 1's full MILP+NLP. Eliminates tau
analytically to get a jointly convex F(lambda,s), then runs Spectral 
Projected Gradient with an exact 2-variable QCQP projection
"""
...
def phi_prime(tau: float) -> float:
    """phi'(tau) for phi(tau) = tau/(1 - sqrt(tau)), analytic derivative."""
...
def r_prime(inst: EVInstance, j: str, lam: float) -> float:
    """r_j'(lambda_j) for r_j(lambda) = p_j*lambda - b_j*lambda^2."""
...
def station_objective(inst: EVInstance, j: str, lam: float, s: float) -> float:
    """F(lambda, s) = kappa_j*s + v_j*phi(lambda^2/s) - r_j(lambda), tau eliminated."""
...
def station_gradient(inst: EVInstance, j: str, lam: float, s: float):
    """Gradient of F(lambda, s) via the chain rule through tau(lambda, s) = lambda^2/s."""
    tau = _tau_of(lam, s)
...
def verify_reduced_objective_convex(n_samples: int = 3000, seed: int = 0) -> bool:
    """Numerically checks F(lambda, s) is jointly convex via random Hessian
    sampling up to ~99% utilization."""
...
def project_onto_feasible_set(lam_hat: float, s_hat: float, lambda_bar_j: float,
                               M_j: float, eps: float):
    """Orthogonal projection of (lam_hat, s_hat) onto {0 <= lam <= lambda_bar_j,
    (lam+eps)^2 <= s <= M_j^2} (station assumed open); a tiny QCQP via
    cvxpy/CLARABEL. Returns None on solver failure so the caller can backtrack."""
...
def reoptimize_station(inst: EVInstance, j: str, xj: int, lambda_bar_j: float,
    ...
    """Algorithm 3 for one station: fixed x_j, updated lambda_bar_j, via
    Spectral Projected Gradient (per-coordinate Barzilai-Borwein + backtracking)."""
    ...
    # no secant pair yet on iter 1, so use a small conservative default step.
    ...
    for k in range(1, max_iter + 1):
        ...
        # per-coordinate Barzilai-Borwein step from the (Delta z, Delta g) secant pair.
        if k > 1:
        ...
        # project the trial point; backtrack (halve t) on a failed projection or non-improvement.
        t = t_diag.copy()
        for _ in range(60):
        ...
        # stop once objective improvement is negligible.
...
def reoptimize_network(inst: EVInstance, x: dict, lambda_bar: dict, **kwargs):
    """Runs Algorithm 3 independently per station (separable given fixed x, lambda_bar)."""
    results = {}
    for j in inst.sites:
...
if __name__ == "__main__":
    ...
    # fixed topology from Algorithm 1's optimal solution, with its routed-demand values.
    x_fixed = {"A": 1, "B": 1, "C": 0, "D": 1, "E": 1}
    lambda_bar_baseline = {"A": 80.00, "B": 74.38, "C": 0.0, "D": 77.57, "E": 118.06}

    print("=================== Algorithm 3: baseline re-optimization "
          "===================")
    print("Fixed topology (from Algorithm 1): stations",
          [j for j in inst.sites if x_fixed[j] == 1], "open.\n")
    ...
    for name, lb in scenarios.items():
        t0 = time.perf_counter()
        # warm-start from the previous scenario's operating point.
        res = {}
        for j in inst.sites:
            z0 = None
