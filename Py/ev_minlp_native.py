"""
Solves the native EV charging network design MINLP, (P-Native), from
"Hidden Convexity for Integrated EV Charging Network Design".

    min_{x,y,lambda,lambda_bar,mu}
        sum_j f_j x_j
      + sum_i sum_j d_ij Lambda_i y_ij
      + sum_j [ c_j(mu_j) + v_j Q_inf,j(lambda_j, mu_j) - r_j(lambda_j) ]

    s.t. sum_j y_ij <= 1                       for all i
         y_ij <= x_j                           for all i, j
         lambda_bar_j = sum_i Lambda_i y_ij     for all j
         0 <= lambda_j <= lambda_bar_j          for all j
         lambda_j + eps * x_j <= mu_j           for all j
         0 <= mu_j <= M_j x_j                   for all j
         x_j in {0,1},  y_ij in [0,1]

with the M/M/1 queueing term

    Q_inf,j(lambda_j, mu_j) = lambda_j^2 / (mu_j * (mu_j - lambda_j)).

This is exactly the problem the paper proves is nonconvex in the native
(lambda, mu) coordinates (Proposition 2.1) -- that nonconvexity lives ONLY
in the objective, since every constraint above is linear in
(x, y, lambda, lambda_bar, mu). That structural fact is what makes a
direct, dependency-light solution approach possible:

  1. The binary station-opening pattern x in {0,1}^|J| is enumerated
     directly (2^|J| combinations -- fine for small/medium |J|, and it is
     exactly what the paper's Algorithm 2 "monolithic" benchmark and the
     master problem of Algorithm 1 ultimately branch over).
  2. For a *fixed* opening pattern, the remaining problem in
     (y, lambda, mu) is a smooth nonconvex NLP with only linear
     constraints. It is solved with SciPy's SLSQP, using several random
     restarts to guard against local optima (the queueing term is
     nonconvex, cf. Proposition 2.1).
  3. The station stability margin eps > 0 (mu_j >= lambda_j + eps) keeps
     the denominator mu_j (mu_j - lambda_j) bounded away from 0, so the
     queueing term never blows up during the search -- this is precisely
     the role Table 1 assigns to eps in the paper.

Only numpy and scipy are required (no external MINLP/NLP solver binaries).

For a convex, globally-solvable reformulation of the SAME problem
(no enumeration needed, provably global optimum via SOCP), see the
hidden-convexity reformulation (P-HC) in Section 3 of the paper --
happy to implement that one too (e.g. with cvxpy) if useful.
"""
...
# trust-constr emits a benign "delta_grad == 0" warning on some restarts
# where the finite-difference Hessian update briefly sees a flat step;
# it does not affect correctness of the reported solution.
...
class EVInstance:
    ...
    def r(self, j, lam_j):
        ...
        (satisfies Assumption 3.1 -- standard diminishing-marginal-revenue
        pricing). This is what makes throttling (lambda_j < lambda_bar_j)
        potentially attractive even before capacity runs out."""
...
def example_instance() -> EVInstance:
    ...
    Lambda = {"Z1": 80.0, "Z2": 60.0, "Z3": 50.0}

    d = {
        ("Z1", "A"): 2.0, ("Z1", "B"): 5.0, ("Z1", "C"): 9.0,
        ("Z2", "A"): 6.0, ("Z2", "B"): 2.0, ("Z2", "C"): 7.0,
        ("Z3", "A"): 8.0, ("Z3", "B"): 6.0, ("Z3", "C"): 3.0,
    }

    f = {"A": 400.0, "B": 350.0, "C": 500.0}
    M = {"A": 130.0, "B": 110.0, "C": 120.0}
    v = {"A": 15.0, "B": 15.0, "C": 15.0}
    kappa = {"A": 0.02, "B": 0.022, "C": 0.02}
    p = {"A": 18.0, "B": 18.5, "C": 16.5}
    b = {"A": 0.03, "B": 0.03, "C": 0.03}
...
def solve_station_subproblem(inst: EVInstance, S: tuple, n_restarts: int = 8):
    """
    Solve the continuous part of (P-Native) with x_j = 1 for j in S and
    x_j = 0 otherwise. Returns (total_cost, detail_dict) or (inf, None) if
    no open site (handled analytically: closing everything costs 0).
    """
    I, J = inst.zones, inst.sites
    nI, nS = len(I), len(S)

    if nS == 0:
        # x = 0 everywhere is always feasible (Proposition 4.2, "complete
        # recourse"): y = 0, lambda = 0, mu = 0 -> objective contribution 0.
    ...
    def objective(z):
        ...
        for j_pos, j in enumerate(S):
            ...
            if denom <= 1e-9:
                # Guard against the singularity mu_j -> lambda_j; keeps the
                # optimizer's line search well-defined even if it briefly
                # probes an (almost) infeasible point.
                return 1e12
    ...
    # --- linear constraints, expressed as a single A z <= b system ---
    # All constraints in (P-Native) besides the queueing/capacity-cost
    # objective are linear, so we hand SciPy exact LinearConstraint /
    # Bounds objects. This is far more numerically robust than SLSQP's
    # dict-of-callables interface, which can stall with a "positive
    # directional derivative" line-search failure on problems like this
    # one where the objective is only *locally* well behaved.
    n_vars = n_y + 2 * nS
    rows, lb, ub = [], [], []
    ...
    for trial in range(n_restarts):
        ...
        # trust-constr's own convergence flag can be conservative; accept
        # any candidate that is (numerically) feasible and improves on the
        # incumbent, rather than relying solely on res.status.
